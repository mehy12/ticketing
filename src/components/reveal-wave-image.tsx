...
interface ImagePlaneProps {
  ...
  externalMouse?: { x: number; y: number } | null;
}
...
function ImagePlane({
  ...
  const scale = useMemo<[number, number, number]>(() => {
    // Cover-fit: fill the entire viewport, cropping if needed
    const viewAspect = viewport.width / viewport.height;
    if (aspectRatio > viewAspect) {
      // Image is wider than viewport — match height, overflow width
      const s = viewport.height;
      return [s * aspectRatio, s, 1];
    } else {
      // Image is taller than viewport — match width, overflow height
      const s = viewport.width;
      return [s, s / aspectRatio, 1];
    }
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      if (hasEnteredRef.current) {
        if (externalMouse) {
          // Use externally provided mouse coordinates (for global tracking)
          material.uniforms.uMouse.value.set(externalMouse.x, externalMouse.y);
        } else {
          // Use Three.js pointer (default behavior)
...
interface RevealWaveImageProps {
  ...
  mouseRadius?: number;
  className?: string;
  /** When true, tracks mouse globally via window events — use when the component is behind other content layers */
...
export const RevealWaveImage = ({
  ...
  const [externalMouse, setExternalMouse] = useState<{ x: number; y: number } | null>(null);
  ...
  // Global mouse tracking for background mode
  useEffect(() => {
    if (!trackMouseGlobally) return;

    const handleMouseMove = (e: MouseEvent) => {
      if (!containerRef.current) return;
      const rect = containerRef.current.getBoundingClientRect();
      // Normalize mouse position to 0-1 UV space relative to the container
      const x = (e.clientX - rect.left) / rect.width;
      const y = 1 - (e.clientY - rect.top) / rect.height; // Flip Y for GL
      setExternalMouse({ x, y });
  ...
  return (
    <div
      ...
      onMouseEnter={trackMouseGlobally ? undefined : () => setIsMouseInCanvas(true)}
      onMouseLeave={trackMouseGlobally ? undefined : () => setIsMouseInCanvas(false)}
    >
      {aspectRatio !== null && (
        <Canvas
          style={{
        ...
        >
          <ImagePlane
            ...
            externalMouse={trackMouseGlobally ? externalMouse : undefined}
          />
        </Canvas>
      )}
    </div>
